...
export default function RightSidebar({ user }) {
  ...
  return (
    <aside className="w-[340px] shrink-0 pt-2 space-y-6 sticky top-6 select-none">
      {/* ── Current User ── */}
      <div className="flex items-center justify-between">
        <Link to="/profile" className="flex items-center gap-3 group min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white group-hover:text-[#a8a8a8] transition-colors truncate">{username}</p>
            <p className="text-xs text-[#737373] truncate">{user.name}</p>
          </div>
        </Link>
        <Link to="/profile" className="text-xs font-semibold text-[#0095f6] hover:text-white transition-colors shrink-0 ml-2">
          Switch
        </Link>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold text-[#737373] uppercase tracking-wider">Suggested for you</span>
          <button className="text-xs font-bold text-white hover:text-[#a8a8a8] transition-colors">See all</button>
        </div>
          ...
          {SUGGESTIONS.map((s) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-white truncate hover:underline cursor-pointer">{s.name}</p>
                  <p className="text-[11px] text-[#737373] truncate">{s.sub}</p>
                </div>
              </div>
              <motion.button
                whileTap={{ scale: 0.9 }}
                onClick={() => setFollowing((p) => ({ ...p, [s.id]: !p[s.id] }))}
                className={`ml-3 text-xs font-bold shrink-0 transition-colors duration-150 ${
                  following[s.id]
